...
function render() {
  ...
  // TODO: Derive scaling and translation factor from the mesh bounding box
  if (overrideShapes) {
    // Clone the shapes to be able to mutate them
    ...
    for (const shape of shapes) {
      switch (shape.type) {
        case 'edge': {
          throw new Error('Unexpected edge shape in OBJ.');
        }
        case 'face': {
  ...
  // Default to the cube mesh
  else {
    const center = [0, 0, -9];
    const rotation = [t, t, t];

    const leftTopBack = rotate([-1, 1, -10], center, rotation);
    const leftTopFront = rotate([-1, 1, -8], center, rotation);
    const leftBottomBack = rotate([-1, -1, -10], center, rotation);
    const leftBottomFront = rotate([-1, -1, -8], center, rotation);
    const rightBottomBack = rotate([1, -1, -10], center, rotation);
    const rightBottomFront = rotate([1, -1, -8], center, rotation);
    const rightTopBack = rotate([1, 1, -10], center, rotation);
    const rightTopFront = rotate([1, 1, -8], center, rotation);

    shapes = [
      { type: 'edge', from: leftTopBack, to: leftTopFront },
      { type: 'edge', from: leftBottomBack, to: leftBottomFront },
      { type: 'edge', from: rightBottomBack, to: rightBottomFront },
      { type: 'edge', from: rightTopBack, to: rightTopFront },
      { type: 'face', points: [leftTopBack, leftBottomBack, rightBottomBack, rightTopBack] },
      { type: 'face', points: [leftTopFront, leftBottomFront, rightBottomFront, rightTopFront] },
    ];
  }
...
export function mount(_overrideShapes) {
  overrideShapes = _overrideShapes;
